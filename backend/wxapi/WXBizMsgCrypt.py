...
class SHA1:
    ...
    """计算公众平台的消息签名接口"""
    def getSHA1_without_data(self, token, timestamp, nonce):
        """用SHA1算法生成安全签名
        @param token:  票据
        @param timestamp: 时间戳
        @param nonce: 随机字符串
...
class WXBizMsgCrypt(object):
    ...
    def __init__(self, sToken, sEncodingAESKey, sAppId, sAppSecret):
        try:
        ...
        self.appsecret = sAppSecret.encode()
    ...
    def VerifyURL(self, sMsgSignature, sTimeStamp, sNonce, sEchoStr):
        sha1 = SHA1()
        ret,signature = sha1.getSHA1_without_data(self.token, sTimeStamp, sNonce)
        if ret  != 0:
            return ret, None
